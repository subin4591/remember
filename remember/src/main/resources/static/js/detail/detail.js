...
$(document).ready(function() {
	...
	request1.open('GET', `https://e-gonghun.mpva.go.kr/opnAPI/publicReportList.do?nPageIndex=1&nCountPerPage=1&type=JSON&mngNo=${param[1]}`);
	request1.send();
	request1.onload = function() {
		...
		img.innerHTML += `<img id="profileImg" src="https://e-gonghun.mpva.go.kr/hise/ua/getImage.do?mngNo=${items.MNG_NO}&type=CH" onerror="this.onerror=null; this.src='${param[0]}/image/${items.SEX == "여" ? "female" : "male"}.svg';" />`;
		
		let article = document.querySelectorAll("article")[0]
		article.innerHTML = `${items.ACHIVEMENT_KO.replaceAll("(", "<span class='hanja'>(").replaceAll(")", ")</span>")}`;
	...
	request2.open('GET', `https://e-gonghun.mpva.go.kr/opnAPI/indepCrusaderList.do?nPageIndex=1&nCountPerPage=1&type=JSON&mngNo=${param[1]}`);
	request2.send();
